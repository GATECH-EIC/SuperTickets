...
from pruners import *
from prune_utils import load_pruner, masked_parameters, prune_loop

import collections
from collections import OrderedDict

from models.hrnet import InvertedResidual, get_block_wrapper
import cv2
...
def print_stats(model):
    if udist.is_master():
        if FLAGS.dataset == 'ade20k':
            logging.info('Num of Params: {}'.format(model.n_params))
            logging.info('Num of MACs: {}'.format(model.n_macs * 512 * 512 / 224 / 224))
        elif FLAGS.dataset == 'cityscapes':
            logging.info('Num of Params: {}'.format(model.n_params))
            logging.info('Num of MACs: {}'.format(model.n_macs * 1024 * 512 / 224 / 224))


    remaining_params, total_params = 0, 0
    for mask, _ in masked_parameters(model,
                                      FLAGS.unstructured_prune_params['prune_bias'],
                                      FLAGS.unstructured_prune_params['prune_batchnorm']
                                    ):
    ...
    if udist.is_master():
        if FLAGS.dataset == 'ade20k':
            logging.info('Num of remaining params: {}; Num of total_params: {}'.format(remaining_params, total_params))
            logging.info('Num of sparse Params: {}'.format(model.n_params - total_params + remaining_params))
            logging.info('Num of sparse MACs: {}'.format(model.n_macs * 512 * 512 / 224 / 224 * remaining_params / total_params))
        elif FLAGS.dataset == 'cityscapes':
            logging.info('Num of remaining params: {}; Num of total_params: {}'.format(remaining_params, total_params))
            logging.info('Num of sparse Params: {}'.format(model.n_params - total_params + remaining_params))
            logging.info('Num of sparse MACs: {}'.format(model.n_macs * 1024 * 512 / 224 / 224 * remaining_params / total_params))


def convert_layers(model, model_ref, layer_type_old, layer_type_new, convert_weights=False):
    conversion_count = 0
    for (name, module), (name_ref, module_ref) in zip(reversed(model._modules.items()), reversed(model_ref._modules.items())):
        if module is None:
            continue
        if len(list(module.children())) > 0:
            # recurse
            model._modules[name] = convert_layers(module, module_ref, layer_type_old, layer_type_new, convert_weights)

        if type(module) == layer_type_old:
            layer_old = module
            layer_new = layer_type_new(module_ref.in_channels, module_ref.out_channels, module_ref.kernel_size, module_ref.stride,
                                             module_ref.padding, module_ref.dilation, module_ref.groups,
                                             module_ref.bias is not None, module_ref.padding_mode).cuda()
...
def run_one_epoch(epoch,
    ...
    for batch_idx, data in enumerate(data_fetcher):
        ...
        if train:
            ...
            # disabled for transfer learning
...
def train_val_test():
    ...
    if FLAGS.resume == '':
        ...
        logging.info('save init at : {}'.format(FLAGS.log_dir))
        save_status(model_wrapper, None, optimizer, ema, 0,
                    1, (None, None),
                    os.path.join(FLAGS.log_dir, 'init_checkpoint'))

    # check pretrained
    if FLAGS.pretrained:
        # print(FLAGS.pretrained)
        # exit()
        checkpoint = torch.load(FLAGS.pretrained,
                                map_location=lambda storage, loc: storage)

        # torch.save(checkpoint, 'cls_imagenet_0.8.pt', _use_new_zipfile_serialization=False)

        # exit()

        new_model_wrapper = checkpoint['model'].cuda()
        if FLAGS.cur_model:
            checkpoint = torch.load(FLAGS.cur_model,
                                    map_location=lambda storage, loc: storage)
            cur_model_wrapper = checkpoint['model'].cuda()


        # new_model = new_model_wrapper.module
        # disable ema
        # if ema:
        ...
        # freeze
        if FLAGS.freeze is True:
            print('freezing...')
            for child in new_model_wrapper.module.children():
                for param in child.parameters():
                    param.requires_grad = False

        # do not load classifier

        # if udist.is_master():
        #     for name, module in new_model_wrapper.module._modules.items():
        #         if 'transform' in name:
        #             for _name, _module in module._modules.items():
        #                 print(_name)

        # exit()

        if udist.is_master():
            print(new_model_wrapper.module.classifier)
            print(model.transform)
            print(model.classifier)
            print('exchange classifier')
        if not isinstance(new_model_wrapper.module.classifier, Sequential):
            _in = new_model_wrapper.module.classifier.in_channels
            _out = model.classifier.out_channels
        else:
            _in = new_model_wrapper.module.classifier[1].in_features
            _out = model.classifier.out_channels
            # print("哈哈哈", _in, _out)

        # new_model_wrapper.module.transform = model.transform

        # name = 'transform'
        # new_model_wrapper._modules[name] = model_wrapper._modules[name]
        # new_model_wrapper.module.transform = convert_layers(new_model_wrapper.module.transform, model.transform, torch.nn.Conv2d, torch.nn.Conv2d)


        # new_model_wrapper.module.classifier = model.classifier
        # new_model_wrapper.module.classifier = torch.nn.Sequential(
        #                                         torch.nn.Conv2d(_in, _out, kernel_size=1),
        #                                         model.classifier
        #                                     )
        
        # transform = InvertedResidual(270, 1600, expand_ratio=4,
        #                 kernel_sizes=[3,5,7],
        #                 stride=1,
        #                 batch_norm_kwargs={'momentum': 0.01, 'eps': 0.001},
        #                 active_fn=torch.nn.ReLU)
        transform = cur_model_wrapper.module.transform
        # transform.output_dim = 1600
        new_model_wrapper.module.transform = transform
        new_model_wrapper.module.classifier = torch.nn.Conv2d(transform.output_dim, _out, kernel_size=1)
        new_model_wrapper.module.task = "segmentation"

        # print(new_model_wrapper.module.classifier)
        # exit()

        if udist.is_master():
            n_parameters = sum(p.numel() for p in new_model_wrapper.module.parameters() if p.requires_grad)
        ...
        model.features = model.features[0: -1]
        # print("模型", model)
        # exit()

        del new_model_wrapper
        ...
        # optimizer = optim.get_unfreezed_optimizer(model_wrapper, FLAGS)
        optimizer = optim.get_optimizer(model_wrapper, FLAGS)
    ...
    if FLAGS.resume:
        ...
        # New
        # FLAGS.num_epochs = last_epoch + 51
    ...
    # for epoch in range(last_epoch + 1, last_epoch + 11): # retrain (w/o grow) 10 epochs right after pruning

        if udist.is_master():
            n_parameters = sum(p.numel() for p in model_wrapper.module.parameters() if p.requires_grad)
            print('unfreezed params: {}'.format(n_parameters))

        print_stats(model_wrapper.module)
        ...
        if ((epoch + 1) % FLAGS.eval_interval == 0) or ((epoch + 1) == FLAGS.num_epochs):
            # val
            ...
            if udist.is_master():
                ...
                save_status(model_wrapper, model_kwparams, optimizer, ema, epoch,
                            ...
                            os.path.join(FLAGS.log_dir, 'retrain_checkpoint'))

    # validate
    # test the sparsity actually meet the requirement
    model = model_wrapper.module
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    prune_ckpt = model.state_dict()

    if udist.is_master():
        # print(prune_ckpt.keys())
        for key in prune_ckpt.keys():
            if 'mask' in key and 'bias' not in key:
                prune_tensor = prune_ckpt[key]
                # print('key: {}'.format(key))
                # print('specific mask: total params {}; nonzero params {}'.format(prune_tensor.numel(), torch.nonzero(prune_tensor).size()))
                # print(torch.flatten(prune_tensor)[:18])
                # print(prune_tensor.requires_grad)
