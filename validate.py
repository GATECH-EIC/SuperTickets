...
from prune_utils import masked_parameters
...
def train_val_test():
    ...
    if FLAGS.profiling:
        if 'gpu' in FLAGS.profiling:
            mc.profiling(model, use_cuda=True, verbose=True)
    ...
    if udist.is_master():
        if FLAGS.dataset == 'ade20k':
            ...
            logging.info('Num of Params: {}'.format(model.n_params))
            logging.info('Num of MACs: {}'.format(model.n_macs))
        elif FLAGS.dataset == 'coco':
            logging.info('Num of Params: {}'.format(model.n_params))
            logging.info('Num of MACs: {}'.format(model.n_macs * 256 * 192 / 224 / 224))
    ...
    if udist.is_master():
        if FLAGS.dataset == 'ade20k':
            ...
            logging.info('Num of remaining params: {}; Num of total_params: {}'.format(remaining_params, total_params))
            logging.info('Num of sparse Params: {}'.format(model.n_params - total_params + remaining_params))
            logging.info('Num of sparse MACs: {}'.format(model.n_macs * remaining_params / total_params))
        elif FLAGS.dataset == 'coco':
            logging.info('Num of remaining params: {}; Num of total_params: {}'.format(remaining_params, total_params))
            logging.info('Num of sparse Params: {}'.format(model.n_params - total_params + remaining_params))
            logging.info('Num of sparse MACs: {}'.format(model.n_macs * 256 * 192 / 224 / 224 * remaining_params / total_params))
    return
...
def main():
    ...
    if udist.is_master():
        FLAGS.log_dir = '{}/{}-test'.format(FLAGS.log_dir,
