...
def train_val_test():
    ...
    for epoch in range(last_epoch + 1, FLAGS.num_epochs):
        ...
        if (epoch + 1) % FLAGS.unstructured_prune_params['prune_interval'] == 0:
            if FLAGS.unstructured_prune_params['schedule'] == 'gradually' and FLAGS.unstructured_prune_params['method'] is not None:
                unstructured_prune_ratio = (epoch + 1) // FLAGS.unstructured_prune_params['prune_interval'] / 10
                if unstructured_prune_ratio <= FLAGS.unstructured_prune_params['target_ratio']:
                    pass
                else:
                    unstructured_prune_ratio = FLAGS.unstructured_prune_params['target_ratio']


                # apply unstructured pruning

                pruner = load_pruner(FLAGS.unstructured_prune_params['method'])(
                    masked_parameters(model,
                ...
                if udist.is_master():
                    logging.info('Start testing at sparsity: {}%.'.format(str(unstructured_prune_ratio*100)))
            ...
            if udist.is_master():
                if FLAGS.model_kwparams.task == 'classification':
                    cur_val = results['top1_error']
                    logging.info('top1 error after pruning: {:.4f}'.format(cur_val))

                elif FLAGS.model_kwparams.task == 'segmentation' and FLAGS.dataset != 'coco':
                    cur_val = results['mIoU']
                    logging.info('New seg mIoU after pruning: {:.4f}'.format(cur_val))

                elif FLAGS.dataset == 'coco':
                    cur_val = results
                    logging.info('New Result after pruning: {:.4f}'.format(cur_val))


            save_status(model_wrapper, model_kwparams, optimizer, ema, epoch,
                        best_val, (train_meters, val_meters),
                        os.path.join(FLAGS.log_dir, 'pruned_checkpoint'))
            ...
            save_status(model_wrapper, model_kwparams, optimizer, ema, epoch,
                        best_val, (train_meters, val_meters),
                        os.path.join(FLAGS.log_dir, 'reset_checkpoint'))
